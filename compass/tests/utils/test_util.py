...
class TestDictMerge(unittest2.TestCase):
    ...
    def test_lhs_rhs_notdict(self):
        lhs = [1, 2, 3]
        rhs = {1: 2}
        self.assertRaises(TypeError, util.mergeDict, (lhs, rhs))
        lhs = {1: 2}
        rhs = [1, 2, 3]
        self.assertRaises(TypeError, util.mergeDict, (lhs, rhs))
...
class TestOrderKeys(unittest2.TestCase):
    def test_simple_order_keys(self):
        keys = [1, 2, 3, 4, 5]
        orders = [3, 4, 5]
        ordered_keys = util.orderKeys(keys, orders)
    ...
    def test_keys_orders_notlist(self):
        keys = {1: 1}
        orders = [3, 4, 5]
        self.assertRaises(TypeError, util.orderKeys, keys, orders)
...
class TestGetListWithPossibility(unittest2.TestCase):
    def test_simple_case(self):
        ...
        self.assertEqual(util.getListWithPossibility(lists),
                         ['role1', 'role2', 'role3'])
